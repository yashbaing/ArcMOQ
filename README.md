...
### Run locally (full stack on port 5173)
...
```bash
npm run arc:status   # Check Arc RPC connection and deployer balance
npm run arc:setup    # Whitelist suppliers, approve KYB, create on-chain order
```
...
| GET | `/api/chain/status` | Arc Testnet connection and deployer status |
| POST | `/api/chain/setup` | Whitelist suppliers, approve KYB, create on-chain order |
...
The build compiles `shared`, `backend`, and `frontend`, copies the UI into `public/`, and runs the Express API via `api/index.ts`. Static assets are served from `public/`; API routes live at `/api/*`.
