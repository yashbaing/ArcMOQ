...
export async function setupDemoOnChain(): Promise<Record<string, string>> {
  ...
  for (const buyer of buyers) {
    await walletClient.writeContract({ address: groupOrder, abi: GroupOrderAbi, functionName: 'setKybApproved', args: [buyer, true] });
    await walletClient.writeContract({ address: receipt, abi: WarehouseReceiptAbi, functionName: 'setKybApproved', args: [buyer, true] });
  }
