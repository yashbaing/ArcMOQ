  ...
  return (
    <div className="progress-rail" role="list">
      {steps.map((step, i) => {
        ...
        return (
          <div key={step} className={`progress-rail__step ${done ? 'done' : ''} ${active ? 'active' : ''}`} role="listitem">
            <div className="progress-rail__dot">{done ? '✓' : i + 1}</div>
...
export function MoqBar({ demand, originalMoq, negotiatedMoq }: { demand: number; originalMoq: number; negotiatedMoq?: number }) {
  const pct = Math.min(100, (demand / originalMoq) * 100);
  const negotiated = negotiatedMoq && negotiatedMoq < originalMoq;
  return (
    <div className="moq-bar">
      <div className="moq-bar__header">
        <span>Demand vs supplier MOQ</span>
        <span className="moq-bar__nums">
          <strong>{demand}</strong> / {negotiated ? <s>{originalMoq}</s> : originalMoq} tins
        </span>
      </div>
      <div className="moq-bar__track">
        <div className="moq-bar__fill" style={{ width: `${pct}%` }} />
        {negotiated && (
          <div className="moq-bar__marker" style={{ left: `${(negotiatedMoq! / originalMoq) * 100}%` }} title={`Negotiated MOQ: ${negotiatedMoq}`} />
        )}
      </div>
      {negotiated && (
...
export function FlowPipeline({ steps }: { steps: Array<{ title: string; detail: string; badge?: ReactNode }> }) {
  return (
    <div className="flow-pipeline">
      {steps.map((step, i) => (
        <div key={step.title} className="flow-pipeline__step">
          <div className="flow-pipeline__num">{i + 1}</div>
          <div className="flow-pipeline__body">
            <div className="flow-pipeline__title">
